...
As soon as the course is confirmed, The Coordinator asks the Admin to book a room for minimum 30 participants and then update the [coordination-tasks-list.xlsx](https://sibcloud-my.sharepoint.com/:x:/g/personal/patricia_palagi_sib_swiss/EZneXy5SD7lApYqGNzj8vsMB8b67OHWWA3NJgfTcgzvFoA?e=9zFWlL) when the booking is confirmed. 
...
Admin should prepare a Form to collect information on the number of attendees and any dietary requirements (e.g., allergies, intolerances, vegan options). After the registration deadline, usually one week before the event, Admin must confirm the number of participants and any dietary requirements with the venue and cc the Coordinator on this communication.

### Trainer Hotel
If a Trainer is coming from another city, the Coordinator will ask the Admin to book a hotel room. Once the booking is confirmed, Admin must send a copy of the bill to *SIB Training Director* and *Senior Administrative Assistant*. Then, Admin must send the confirmation of booking to the Trainer and the Coordinator.  

### Coffee Breaks
D-10: Admin must confirm with the Coordinator the number of participants to the course and then order coffee breaks (morning and afternoon). When the order is confirmed, Admin must notify the Coordinator.  
...
When Admin receives the OK from Coordinator, Admin creates the feedbacks to be sent to the Trainers (Export in pdf format the answers of the participants - The link is on line 30 of the task list - then manually insert the answers to questions 10-11-12). In addition, Admin must create the feedbacks to be uploaded on elixir (Demographic and quality metrics) by using the templates.  
...
**RT**: Admin reads the emails. If Admin has the answer, Admin will take the ticket and answer it. If not, Admin will assign it to the appropriate person (usually the Coordinator of the course). There is a lot of SPAM. For those mails the procedure is the following: Bulk update > select mails > scroll down > make queue > choose SPAM > save. 

**Editing Videos**: 
- Coordinator sends to Admin the raw video and the exact timing of the videos to cut. Once Admin finished editing, Admin puts the videos on the cloud.  
- Youtube: Once Coordinator confirms that the edited videos are good, Admin uploads the video on YouTube, following the steps of the Tutorial.  
...
**Short-Term Feedback**: Once the feedback is closed, Coordinator informs Admin that he can share the last version of the feedback in pdf and a summary of questions 10, 11 and 12 (extracted from the excel) with the Trainers. To create the feedback, Admin export the online feedback in PDF format, then using Adobe Acrobat add manually the answers to question 10, 11 and 12 of the feedback.  
